...
from app import app, db, Country, Government, Doctrine, Inventory, Region, Structure, Stats, Technology, CountryTechnology, User, StructureData, StructureProduction, TechnologyAttribute, TechnologyLicense, CountryTechnologyInventory, CountryTechnologyProduction, CountryDoctrine, GameDate, PlaydaysPerMonth, ServerSettings
...
def test_complex_queries():
    ...
    with app.app_context():
        try:
            ...
            total_playdays = db.session.query(db.func.sum(PlaydaysPerMonth.playdays)).scalar() or 0
...
if __name__ == "__main__":
    ...
    error_count = sum(1 for v in results.values() if isinstance(v, str) and "ERROR" in v)
